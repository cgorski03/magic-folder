...
namespace magic_core
{
  ...
  OllamaClient::OllamaClient(OllamaClient &&other) noexcept
      : ollama_url_(std::move(other.ollama_url_)), embedding_model_(std::move(other.embedding_model_)), curl_handle_(other.curl_handle_)
  {
  ...
  std::vector<float> OllamaClient::get_embedding(const std::string &text)
  {
    ...
    curl_easy_setopt(curl_handle_, CURLOPT_URL, (ollama_url_ + "/api/embeddings").c_str());
