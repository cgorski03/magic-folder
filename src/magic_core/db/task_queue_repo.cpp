...
std::string TaskQueueRepo::time_point_to_string(const std::chrono::system_clock::time_point &tp) { return format_time(tp); }
...
std::vector<Task> TaskQueueRepo::get_tasks_by_status(TaskStatus status) {
  ...
  db_ << "SELECT id, task_type, file_path, status, priority, error_message, created_at, updated_at FROM task_queue WHERE status = ? ORDER BY priority ASC, created_at ASC"
      << status_str >>
      [&](long long id, std::string task_type, std::string file_path, std::string status_db, int priority,
          std::optional<std::string> error_message, std::string created_at, std::string updated_at) {
        Task task;
        task.id = id;
        task.task_type = task_type;
        task.file_path = file_path;
