...
void DatabaseManager::initialize(const std::filesystem::path& db_path, const std::string& db_key, int pool_size) {
    if (is_initialized_) {
        return;
    }
...
void DatabaseManager::setup_schema(const std::filesystem::path& db_path, const std::string& db_key) {
    ...
    if (sqlite3_key(handle, db_key.c_str(), db_key.length()) != SQLITE_OK) {
        throw std::runtime_error("Setup: Failed to key database: " + std::string(sqlite3_errmsg(handle)));
    }
