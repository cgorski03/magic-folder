...
MetadataStore::MetadataStore(MetadataStore &&other) noexcept
    : db_path_(std::move(other.db_path_)), db_(std::move(other.db_)), faiss_index_(other.faiss_index_) {
...
void MetadataStore::initialize() {
  try {
    ...
    // Open database using sqlite-modern-cpp
    ...
    *db_ << "SELECT id FROM files WHERE path = ?" << basic_metadata.path
    ...
    if (existing_id != -1) {
      ...
      *db_ << "UPDATE files SET original_path=?, file_hash=?, processing_status=?, "
              ...
              "summary_vector_blob=NULL, suggested_category=NULL, suggested_filename=NULL WHERE path=?"
           << basic_metadata.original_path
           << basic_metadata.file_hash
           << basic_metadata.processing_status
           << basic_metadata.tags
           << last_modified_str
           << to_string(basic_metadata.file_type)
  ...
  try {
    ...
    *db_ << "SELECT id, path, original_path, file_hash, processing_status, tags, "
         ...
         >> [&](int id, std::string path, std::optional<std::string> original_path,
                ...
                std::optional<std::string> tags, std::string last_modified,
         ...
         >> [&](int64_t id, std::vector<char> vector_blob) {
             ...
             if (vector_blob.size() == VECTOR_DIMENSION * sizeof(float)) {
               ...
               std::cerr << "Warning: Skipping file ID " << id
                         ...
                         << VECTOR_DIMENSION * sizeof(float) << " bytes, got " << vector_blob.size() << " bytes."
                         << std::endl;
             }
         };
