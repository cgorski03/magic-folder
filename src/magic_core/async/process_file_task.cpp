...
ProcessFileTask::ProcessFileTask(long long id, TaskStatus status,
...
void ProcessFileTask::process_chunks_in_batches(
    long long file_id, std::vector<Chunk>& chunks, ServiceProvider& services,
    ...
    for (size_t i = 0; i < chunks.size(); ++i) {
        ...
        std::string message = "Embedding chunk " + std::to_string(i + 1) + " of " + std::to_string(chunks.size());
