...
namespace magic_core
{
  ...
  ExtractedContent ContentExtractor::extract_from_text(const std::string &text, const std::string &filename)
  {
  ...
  FileType ContentExtractor::detect_file_type(const std::filesystem::path &file_path)
  {
    ...
    else if (extension == ".md" || extension == ".markdown")
    {
      return FileType::Markdown;
             ...
             extension == ".java" || extension == ".cs" || extension == ".php" ||
             extension == ".rb" || extension == ".go" || extension == ".rs")
    {
  ...
  std::vector<std::string> ContentExtractor::extract_keywords(const std::string &content)
  {
    ...
    for (std::sregex_iterator i = words_begin; i != words_end; ++i)
    {
      ...
      // Skip common words
      if (word.length() > 3 && word != "the" && word != "and" && word != "for" &&
          word != "with" && word != "this" && word != "that" && word != "from")
      {
        word_count[word]++;
      }
    }
